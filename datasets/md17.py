...
class MD17(InMemoryDataset):
    ...
    def __init__(self, root, cut_dist=5., transform=None, pre_transform=None, dataset_arg=None):
    ...
    def processed_file_names(self):
        return [f"md17-{mol}-{self.cut_dist}.pt" for mol in self.molecules]
    ...
    def process(self):
        for path in self.raw_paths:
            ...
            for pos, y, dy in zip(positions, energies, forces):
                graph = Data(pos=pos, y=y.unsqueeze(1), atom=z, dy=dy, dataset='md17', subdata = self.dataset_arg)
